...
class ColorTheoryLoss(nn.Module):
    ...
    def forward(self, predictions: Dict, targets: Dict) -> Dict[str, torch.Tensor]:
        ...
        if 'skin_tone_probs' in predictions and 'skin_tone_labels' in targets:
            skin_loss = self.ce_loss(predictions['skin_tone_probs'], targets['skin_tone_labels'])
        ...
        if 'undertone_probs' in predictions and 'undertone_labels' in targets:
            undertone_loss = self.ce_loss(predictions['undertone_probs'], targets['undertone_labels'])
        ...
        if 'harmony_probs' in predictions and 'harmony_labels' in targets:
            harmony_loss = self.ce_loss(predictions['harmony_probs'], targets['harmony_labels'])
    ...
    def _color_consistency_loss(self, color_features: torch.Tensor, 
                               target_colors: torch.Tensor) -> torch.Tensor:
        """
...
class MLColorAnalysisPipeline:
    ...
    def analyze_image_complete(self, image: np.ndarray, 
                           existing_wardrobe: List[Dict],
                           occasion: str = 'casual',
                           season: str = 'spring') -> MLColorPrediction:
        """
        ...
        skin_tone, skin_conf = self.color_engine.extract_skin_tone_from_face(face_region)
        ...
        recommendations = self.recommendation_engine.generate_outfit_color_suggestions(
            color_profile=self.recommendation_engine.analyze_complete_color_profile(face_region),
